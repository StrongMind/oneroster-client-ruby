  ...
  class ResultType
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OneRosterClient::ResultType` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OneRosterClient::ResultType`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      score_status_validator = EnumAttributeValidator.new('', ['exempt', 'fully graded', 'not submmitted', 'partially graded', 'submitted'])
    ...
    def hash
      [line_item, student, score_status, score, score_date, comment, sourced_id, status, date_last_modified, metadata].hash
    end
