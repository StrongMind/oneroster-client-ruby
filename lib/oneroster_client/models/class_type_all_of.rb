  ...
  class ClassTypeAllOf
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OneRosterClient::ClassTypeAllOf` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OneRosterClient::ClassTypeAllOf`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      class_type_validator = EnumAttributeValidator.new('Object', ['homeroom', 'scheduled'])
    ...
    def class_type=(class_type)
      validator = EnumAttributeValidator.new('Object', ['homeroom', 'scheduled'])
      unless validator.valid?(class_type)
        fail ArgumentError, "invalid value for \"class_type\", must be one of #{validator.allowable_values}."
      end
    ...
    def hash
      [title, class_code, class_type, location, grades, subjects, course, school, terms, subject_codes, periods, resources].hash
    end
